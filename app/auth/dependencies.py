...
# ── HTTP Bearer scheme ────────────────────────────────────────────────────────
# HTTPBearer tells Swagger UI to show a plain "Value: <token>" input box
# in the Authorize dialog — exactly right for our JWT flow.
# At runtime it extracts the token from: Authorization: Bearer <token>
# The behaviour is identical to OAuth2PasswordBearer for our purposes;
# we just get a cleaner Swagger UI without the irrelevant username/password
# and client_id/client_secret fields.
